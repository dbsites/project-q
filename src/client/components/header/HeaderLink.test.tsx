...
const props = {
  ...
  link: headerLinkData.login.link,
  linkKey: headerLinkData.login.linkKey,
  linkText: headerLinkData.login.linkText,
}
...
describe('HeaderLink Component', () => {
  ...
  it('renders a <li> with key equal to linkKey prop', () => {
    const headerLinkWrapper: Enzyme.ShallowWrapper = Enzyme.shallow(<HeaderLink {...props} />)
  ...
  it('contains a Link to the link prop with text equal to the text prop', () => {
    ...
    expect(headerLinkWrapper.find(Link).props()).toHaveProperty('children', props.linkText)
    expect(headerLinkWrapper.find(Link).props()).toHaveProperty('to', props.link)
  })
});
